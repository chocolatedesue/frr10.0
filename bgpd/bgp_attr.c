...
#define MIX3(a, b, c)	key = jhash_3words((a), (b), (c), key)
	...
	/* BGP link state flipping attributes */
	MIX3(attr->source_router_id, attr->destination_router_id, attr->link_final_state);
...
bool attrhash_cmp(const void *p1, const void *p2)
{
	...
	if (attr1->flag == attr2->flag && attr1->origin == attr2->origin
		...
		if (attr1->aggregator_as == attr2->aggregator_as &&
		    ...
		    attr1->otc == attr2->otc &&
		    /* BGP link state flipping attributes */
		    attr1->source_router_id == attr2->source_router_id &&
		    attr1->destination_router_id == attr2->destination_router_id &&
		    attr1->link_final_state == attr2->link_final_state)
			return true;
	}
				  ...
				  uint8_t origin)
{
	...
	/* Initialize BGP link state flipping attributes */
	attr->source_router_id = 0;
	attr->destination_router_id = 0;
	attr->link_final_state = 0;
...
/* Get link-state flip attribute.
 * This attribute is used to indicate that the link-state information
 * in the UPDATE message is flipped, i.e., the link-state information
 * is not the same as the one in the previous UPDATE message.
 */
static enum bgp_attr_parse_ret
bgp_attr_link_state_flip_parse(struct bgp_attr_parser_args *args)
{
	...
	if (length != 9) {
		flog_err(EC_BGP_ATTR_LEN,
			 "Link-state flip attribute length is not 9 [%u]",
			 length);
		return bgp_attr_malformed(args, BGP_NOTIFY_UPDATE_ATTR_LENG_ERR,
					  args->total);
	}

	uint32_t source_route_id = stream_getl(peer->curr);
	uint32_t destination_route_id = stream_getl(peer->curr);
	uint8_t link_final_state = stream_getc(peer->curr);
    ...
    sprintf(debug_buf, "[%s] Received flip attribute from [%s]: source_route_id: %s, destination_route_id: %s, link_final_state: %u\n",
			local_route_id_str, remote_route_id_str, src_route_id_str, dst_route_id_str, (unsigned int)link_final_state);
...
{
	// char debug_buf[300];
    // sprintf(debug_buf, "local_route_id %u Start to parse BGP attributes for peer %u, size %u\n",
	// 		peer->local_id.s_addr, peer->remote_id.s_addr, size);
    // int fp1 = open("/home/frr/test/test.txt", O_WRONLY | O_APPEND | O_CREAT, 0666);
    // int write_n1 = write(fp1, debug_buf, strlen(debug_buf));
    // close(fp1);
	enum bgp_attr_parse_ret ret;
	...
	while (BGP_INPUT_PNT(peer) < endp) {
		...
		case BGP_ATTR_AIGP:
			...
			break;
		case BGP_ATTR_LINK_STATE_FLIP:
			ret = bgp_attr_link_state_flip_parse(&attr_args);
			break;
		default:
...
{	
	...
	if (source_peer && (source_peer -> is_flip || source_peer -> is_triggered_by_attr)) {
		char local_route_id_str[INET_ADDRSTRLEN], src_route_id_str[INET_ADDRSTRLEN],dst_route_id_str[INET_ADDRSTRLEN],remote_route_id_str[INET_ADDRSTRLEN];
		...
		uint8_t link_final_state = 0;
		if (source_peer -> is_flip) {
	
			char debug_buf2[300];
			// inet_ntop(AF_INET, &(source_peer->remote_id.s_addr), remote
			// route_id_str, sizeof(remote_route_id_str));
			sprintf(debug_buf2, "[%s] Trigger by self flip with [%s]: \n",
		...
		source_peer -> is_triggered_by_attr = 0;

		stream_putc(s, BGP_ATTR_LINK_STATE_FLIP);
		stream_putc(s, 9); // Length
		// 9 = 4 (source_route_id) + 4 (dest_route
		...
		inet_ntop(AF_INET, &destination_route_id, dst_route_id_str, sizeof(dst_route_id_str));
		// Log the detected link-state flip attribute
		sprintf(debug_buf, "[%s] SEND flip attribute to [%s]: source_route_id=%s, destination_route_id=%s, link_final_state=%u, bgp->address: %p, bgp->peer address: %p, source_peer: %p\n",
			local_route_id_str, remote_route_id_str,
			src_route_id_str, dst_route_id_str, link_final_state,
			(void*)bgp ,(void*)bgp->peer, (void*)source_peer);
		int fp1 = open("/home/frr/test/test.txt", O_WRONLY | O_APPEND | O_CREAT, 0666);
		int write_n1 = write(fp1, debug_buf, strlen(debug_buf));
		close(fp1);
	...
	// 	sprintf(debug_buf, "[%s] SEND flip attribute to [%s]: source_route_id=%s, destination_route_id=%s, link_final_state=%u, bgp->address: %p, bgp->peer address: %p, source_peer: %p\n",
	// 			local_route_id_str, remote_route_id_str,
	// 				src_route_id_str, dst_route_id_str, link_final_state, (void*)bgp ,(void*)bgp->peer, (void*)source_peer);
